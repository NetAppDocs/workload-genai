...
+
NOTE: The data guardrails feature only processes .CSV files when enabled for Amazon Q Business connectors.

.. *FSx for ONTAP file system*: When you define a new Amazon Q Business connector, workload factory creates a new Amazon FSx for NetApp ONTAP volume to store the connector information. Choose an existing file system and SVM (also called a storage VM) where the new volume will be created.
...
The data source is embedded into your connector. The status changes from "Embedding" to "Embedded" when the data source is completely embedded.

After you add a single data source to the connector, you can test it in the Amazon Q Business chatbot window and make any required changes before you make the chatbot available to your users. You can also follow the same steps to add additional data sources to the connector.