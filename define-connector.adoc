---
sidebar: sidebar
permalink: define-connector.html
...
= Integrate with Amazon Q Business
:icons: font
:imagesdir: ./media/

[.lead]
After you've deployed the AI infrastructure and identified the data sources that you'll use from your FSx for ONTAP datastores, you are ready to integrate workload factory with Amazon Q Business by defining an Amazon Q Business connector. 
...
+
NOTE: Data guardrails can be enabled or disabled at any time. If you switch data guardrails enablement, workload factory scans the entire data source from scratch, which incurs a cost.

== Define an Amazon Q Business connector
Integrate workload factory with Amazon Q Business by defining a connector. The connector enables API and data source communication between GenAI and Amazon Q Business.
...
+
NOTE: The data guardrails feature is not supported when ingesting structured data files such as CSV, JSON, JSONP, or Parquet. When the data guardrails feature is enabled, GenAI processes .txt, .md, .csv, .docx, and .pdf files by ingesting only plain text (excluding embedded image or media text) and masking any private or sensitive data. All other file types are processed normally without masking private or sensitive data.

.. *FSx for ONTAP file system*: When you define a new Amazon Q Business connector, workload factory creates a new Amazon FSx for NetApp ONTAP volume to store it. Choose an existing file system name and SVM (also called a storage VM) where the new volume will be created.
.. *Snapshot policy*: Choose a snapshot policy from the list of existing policies defined in the workload factory storage inventory. GenAI automatically creates recurring snapshots of the connector at a frequency based on the snapshot policy you select.
+
