---
sidebar: sidebar
permalink: manage-connectors.html
...
. From the Knowledge bases inventory page, select the connector that you want to update.
...
+
NOTE: Every data source scan, which includes embedding, incurs a cost. If you enable data guardrails after a knowledge base has been created, then the data source gets scanned again and incurs costs. 
+
. Select *Save* after you have made changes.

== Protect a connector with snapshots
You can protect your connector data by taking and restoring snapshots of your connector volumes. You can restore from a snapshot to revert to the previous version of the connector at any time.

Snapshots can be faster and more storage-efficient than backups, and enable you to protect each connector using a different protection policy. Some of the scenarios where snapshots can be useful are:

* Accidental data loss or corruption
* Recovering from incorrect data being ingested into the connector
* Testing different data sources or chunking strategies, and quickly reverting when the testing is complete

=== Take a snapshot of a connector volume
You can save the state of a knowledge base by taking a manual snapshot of the connector volume.
...
You can restore a manual or scheduled snapshot of a connector volume at any time.
...
. From the Knowledge bases inventory page, select the connector that you want to restore.
...
+
The snapshot selection dialog appears, where you can see a list of the snapshots that have been created for this connector.
...
+
This option is enabled by default to ensure that a scan doesn't happen while the connector is in a partially restored state, or that a scan doesn't update a freshly restored connector with older data.
...
Data sources are synchronized with the associated connector automatically once a day so that any data source changes are reflected in Amazon Q Business. If you make changes to any of your data sources and you'd like to synchronize the data immediately, you can perform an on-demand synchronization.
...
. From the connector inventory page, select the connector that you want to synchronize.
...
If you no longer need a connector, you can delete it. When you delete a connector, it is removed from workload factory and the volume that contains the connector is deleted. Deleting a knowledge base is not reversible.

When you delete a connector, you should also disassociate the connector from any agents it is associated with to fully delete all resources associated with the knowledge base.
