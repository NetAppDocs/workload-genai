...
* If applicable, select the inference profile that matches the configuration of the selected model.
...
.. *Reranking*: Enable or disable reranking, which can improve the relevance and quality of query results. Choose a standard chat model or a specialized reranker model to use for reranking. Reranker model options are only shown if they are available in your region. Select the inference profile that matches the configuration of the selected model.

.. *Retrieve settings*:
+
* Set a *Reranking score* (available only if reranking is enabled) to configure the ordering of query results based on the reranking score. Higher settings cause results with a higher reranking score to be preferred.
* Choose the number of *Retrieved database documents* to configure how many documents to query from the database for semantic and textual searches. The default value is 5 for each search.
* Choose the number of *Retrieved total documents* to configure the maximum number of documents to return at the end of the retrieval process.
* Choose the *Distance* to configure the exclusion of irrelevant results from a semantic search. A smaller distance instructs the model to only include results with a higher similarity to the query. By default, GenAI includes results with a distance of less than 0.5.
* Select a *Retrieve type* to configure the chatbot to use semantic search (relying on meaning and context of queries) or hybrid search (keyword search combined with semantic search) when generating results. Hybrid search is selected by default.

.. *Retrieve time configuration*:
+
