...
.. *Retrieve settings*:
+
* Set a *Reranking score* to configure the ordering of query results based on the relevance.
* Choose the number of *Retrieved database documents* to control how many documents the model can use to generate responses.
* Choose the number of *Retrieved total documents* to configure how many documents are sent to the chat session.
* Choose the *Distance* to configure the ordering of query results based on their similarity to the context of the original query.
* Select a *Retrieve type* to configure the chatbot to use semantic search (relying on meaning and context of queries) or hybrid search (keyword search combined with semantic search) when generating results.
