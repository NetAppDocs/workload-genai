...
.. *S3 Bucket*: If chatbot query results are complex (containing a table with more than 10 rows), GenAI can store the results in an S3 bucket. To use this feature, enable the *Activate S3 Bucket* setting and choose an S3 bucket that is associated with your account from the list. When complex results are stored in an S3 bucket, you can download them using the download link within the chatbot session.
+
