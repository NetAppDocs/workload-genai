...
.. *FSx for ONTAP file system*: When you define a new NetApp ONTAP Connector for Amazon Q, workload factory creates a new Amazon FSx for NetApp ONTAP volume to store the connector information. Choose an existing file system and SVM (also called a storage VM) where the new volume will be created.
