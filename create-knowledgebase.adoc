...
+
Several prerequisites must be complete to enable data guardrails. 
+
A service account is created to communicate with BlueXP classification. You must have the admin role on your tenancy account for service account creation. 
+
Additionally, to enable data guardrails, you'll need to do the following as described in link:https://docs.netapp.com/us-en/bluexp-classification/task-deploy-cloud-compliance.html#quick-start[BlueXP classification documentation^]: 
+
