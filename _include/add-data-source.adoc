...
+
NOTE: Your choice to include image files in scans is related to the chat model the knowledge base is configured with. To include image files in scans, the chat model must support images. You cannot switch the knowledge base to a chat model that does not support image files while image file types are selected here. 

** In the *File modification time filter* section, choose to enable or disable inclusion of files based on their modification time. If you enable modification time filtering, you need to choose a date range from the list.
+
